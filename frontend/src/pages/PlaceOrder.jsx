...
const PlaceOrder = () => {
      ...
      try {
        ...
        switch (method) {
          case 'cod':
            ...
            if(result.data){
              setCartItem({}) // Clear cart after order placed
              navigate('/order')
            }else{
              console.log("Order failed" + result.data.message)
            }
            break;
  ...
  return (
    <div className='w-[100vw] min-h-[100vh] bg-gradient-to-b from-[#ffe0f0] via-[#eafff6] flex items-center justify-center flex-col md:flex-row gap-[50px]  relative'>
      <div className='lg:w-[50%] w-[100%] h-[100%] flex items-center justify-center  lg:mt-[0px] mt-[90px] '>
        <form onSubmit={onSubmitHandler} action="" className='lg:w-[70%] w-[95%] lg:h-[70%] h-[100%]'>
         ...
         <div>
          <button type='submit' className='text-[18px]  active:bg-slate-500 cursor-pointer bg-black py-[10px] px-[50px] rounded-2xl text-white flex items-center justify-center gap-[20px] absolute  lg:right-[16%] bottom-[5%] right-[25%] border-[1px] border-[#80808049] ml-[30px] mt-[20px]' >PLACE ORDER</button>
         </div>
        </form>
      </div>
      {/* Right */}
